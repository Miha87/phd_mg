# -*- coding: utf-8 -*-
"""
Created on Mon May 11 09:29:55 2020

@author: Mihael Gudlin
"""
...
def video_to_segment(path_to_video, output_dir, output_file, start_point, duration):
    '''Pretvara dugi video u kraći isječak definiranog trajanja bez re-kodiranja - veoma brza transforamacija 
    kroz kopiranje ulaznog stream-a. Originalnom video zapisu dodaje nekoliko crnih sličica ispred i iza, ali kod
    pretvorbe iz videa u sliku, te sličica ffmpeg odbacuje, tako da to nije problem kod označavanja.
    
    Argumenti:
    ---------
    path_to_video : apsolutna putanja do video zapisa
    
    output_dir : apsolutna putanja do direktorija u kojem će biti pohranjen isječak
    
    output_file : naziv datoteke izlaznog isječka
    
    start_point : <"minuta:sekunda"> u kojoj minuti i sekundi kreće isječak (npr. 9:15), tip ulaza je "str"
    
    duration : <"minuta:sekunda"> koliko je ukupno trajanje isječka, tip ulaza je "str"
    '''
...
def video_to_image(path_to_video, output_dir, image_res, fps):
    ...
    image_res : <"širina:visina"> definira rezoluciju izlazne slike
...
def concat_two_videos(path_to_video_1, path_to_video_2, output_dir, output_file):
    ...
    Argumenti:
    ---------
    path_to_video_1 : apsolutna putanja do prvog video zapisa
    
    path_to_video_2 : apsolutna putanja do drugog video zapisa
    
    output_dir : apsolutna putanja do direktorija u kojem će biti pohranjeni spojeni video zapis
    
    output_file : naziv datoteke spojenog video zapisa
    '''
    with open(os.devnull, 'w') as ffmpeg_log: #os.devnull služi za prikupljanje svih printova i njegovo odbacivanje
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        ffmpeg_call = ["ffmpeg",            #poziv s cmd line-a za ffmpeg, ovdje moramo kao ulaz dati "string",kod cmd ne moramo
                       ...
                       "-filter_complex", "concat=n=2:v=1:a=0", #opcija -filter_complex, koristimo opciju concat(spajanje), #n=2 (dva videa), v=1(postoji video), a=0 (ne postoji audio)
...
def images_to_video(path_to_images, start_image, end_image, output_dir, output_file):
    ...
    Argumenti:
    ---------
    path_to_images : apsolutna putanja do slika
    
    start_image : [int] oznaka početne slike
    
    end_image : [int] oznaka završne slike
    
    output_dir : apsolutna putanja do direktorija u kojem će biti pohranjeni spojeni video zapis
    
    output_file : naziv datoteke izlaznog video zapisa 
    '''
    with open(os.devnull, 'w') as ffmpeg_log: #os.devnull služi za prikupljanje svih printova i njegovo odbacivanje
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
...
def video_to_gif(path_to_video, output_dir, output_file, image_res, fps):
    ...
    output_dir : apsolutna putanja do direktorija u kojem će biti pohranjeni gif
    
    output_file : naziv datoteke izlaznog gif-a (npr. "ime.gif")
